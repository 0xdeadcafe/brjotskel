#!/bin/sh
# eradication/linux/remove-profile-hook.sh — Remove malicious shell profile/rc hook
# Requires: root
...
#
# Usage:
#   # Remove lines from a profile file:
#   HOOK_FILE=/etc/profile.d/sysenv.sh HOOK_PATTERN="curl\|wget\|base64" \
#     remote_exec(session="host01", command="<paste>")
#
...
echo ""
echo "Note: Shell hooks only execute on new shell start — existing sessions are not affected."
echo "Verify no active shells will re-exec the hook on next login."
